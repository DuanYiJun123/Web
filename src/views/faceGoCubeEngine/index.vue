      ...
      getCubeData() {
        ...
        getCubeAdvan().then(res => {
          if (res.data.status !== 0) {
            ...
            console.log(this.advanList1);
            this.advanList2 = responce.content.advanList2;
          }
        });

        //  应用场景
        getCubeAppli().then(res => {
          if (res.data.status !== 0) {
            this.$message.error('获取数据失败!请检查！')
          } else {
            let responce = res.data.data;
