...
<script>
  ...
  import {
    getStriationRecogBanner,
    getStriationRecogIntroduce,
    getStriationRecogFun,
    getStriationRecogAdvan,
    getStriationRecogAppli,
    getStriationRecogExper
  } from '@/api/coreTech'
  ...
  export default {
    ...
    methods: {
      getSmartData() {
        // banner 图片
        getStriationRecogBanner().then(res => {
          // console.log(res);
        ...
        //   产品介绍
        getStriationRecogIntroduce().then(res => {
          if (res.data.status !== 0) {
            this.$message.error('获取数据失败!请检查！')
          } else {
            let responce = res.data.data;
        ...
        getStriationRecogFun().then(res => {
          if (res.data.status !== 0) {
            ...
            this.smartFuns.preTitle = bigTitle.slice(0, bigTitle.length / 2+2);
            this.smartFuns.postfixTitle = bigTitle.slice(bigTitle.length / 2+2, bigTitle.length);
        ...
        //   产品优势
        getStriationRecogAdvan().then(res => {
          if (res.data.status !== 0) {
            this.$message.error('获取数据失败!请检查！')
          } else {
            let responce = res.data.data;
        ...
        getStriationRecogExper().then(res => {
          if (res.data.status !== 0) {
            this.$message.error('获取数据失败!请检查！')
          } else {
            let responce = res.data.data;
        ...
        //  应用场景
        getStriationRecogAppli().then(res => {
          if (res.data.status !== 0) {
            this.$message.error('获取数据失败!请检查！')
          } else {
            let responce = res.data.data;
