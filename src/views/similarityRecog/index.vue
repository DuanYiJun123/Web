<template>
  <div class="smart-container">
    <OrderBanner v-bind="banner" v-show="banner"></OrderBanner>
...
<script>
  ...
  import {
    getSimilarityRecogBanner,
    getSimilarityRecogIntroduce,
    getSimilarityRecogFun,
    getSimilarityRecogAdvan,
    getSimilarityRecogAppli,
    getSimilarityRecogExper
  } from '@/api/coreTech'
  ...
  export default {
    ...
    methods: {
      getSmartData() {
        // banner 图片
        getSimilarityRecogBanner().then(res => {
          // console.log(res);
        ...
        //   产品介绍
        getSimilarityRecogIntroduce().then(res => {
          if (res.data.status !== 0) {
            this.$message.error('获取数据失败!请检查！')
          } else {
            let responce = res.data.data;
            ...
            this.smartIntro.preTitle = bigTitle.slice(0, 7);
            this.smartIntro.postfixTitle = bigTitle.slice(7, bigTitle.length);
        ...
        //  产品功能
        getSimilarityRecogFun().then(res => {
          if (res.data.status !== 0) {
            this.$message.error('获取数据失败!请检查！')
          } else {
            let responce = res.data.data;
            ...
            this.smartFuns.preTitle = bigTitle.slice(0, bigTitle.length / 2+1);
            this.smartFuns.postfixTitle = bigTitle.slice(bigTitle.length / 2+1, bigTitle.length);
        ...
        //   产品优势
        getSimilarityRecogAdvan().then(res => {
          if (res.data.status !== 0) {
            this.$message.error('获取数据失败!请检查！')
          } else {
            let responce = res.data.data;
        ...
        getSimilarityRecogExper().then(res => {
          if (res.data.status !== 0) {
            this.$message.error('获取数据失败!请检查！')
          } else {
            let responce = res.data.data;
        ...
        //  应用场景
        getSimilarityRecogAppli().then(res => {
          if (res.data.status !== 0) {
            this.$message.error('获取数据失败!请检查！')
          } else {
            let responce = res.data.data;
