      ...
      getClientData() {
        ...
        getSecCaseTypical().then(res => {
          ...
          if (res.data.status !== 0) {
            ...
            // this.caseList= lists;
            // console.log(lists);
          var a = lists.map(item => {

              this.itemList.useProduct = item.product.slice(0, item.product.indexOf("：")+1);
              this.itemList.productName = item.product.slice(item.product.indexOf("：")+1, item.product.length);
